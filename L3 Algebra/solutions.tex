...
illustrated, is (translated of course):\footnote{~Translation from \cite{Ste08}.}
...
The complex numbers are generally considered to have been formally born in Girolamo Cardano's \emph{Ars Magna} in 1545, where he
...
existance of complex numbers for two main reasons: firstly they do not introduce any contradictions into basic arithmetic or
any theory that we care about, and secondly they allow us to state the following historic theorem.

\begin{thm}[Fundamental Theorem of Algebra]
  Let $ p(x) $ be a polynomial with degree $ d $. Then, counting repeated roots, there are exactly $ d $ complex roots of $ p(x) $.
\end{thm}

The proof of the Fundamental Theorem unfortunately requires concepts and techniques far beyond the scope of this book (Stewart
gives a proof in \cite{Ste15}, and Artin gives two(!) proofs in \cite{Art91} \S 9.9). All proofs (that I am aware
of) require the use of analysis (i.e. there is no pure-algebra proof).

The first rigorous proof of this theorem was published by French mathematician Jean-Robert Argand in 1814; the name is somewhat
...
The five points marked on this Argand diagram are $ i $, 1, $ 3+2i $, $ -1 - i $, and $ 2i - 1 $. See if you can label each point on the diagram itself.
...
and in the general case as exercise 9.\ref{ex:solutiongenerator}).
...
(known as the \emph{argument} of $ z $, and written as $ \carg(z) $).
...
It is possible to make a sensible definition for complex exponents using \emph{Euler's formula},
\begin{displaymath}
  \cis \theta = e^{i\theta}.
\end{displaymath}
...
As a short aside, we have already seen that we can view the complex numbers as points in the plane, $ \mathbb{R}^2 $. This means
...
\begin{enumerate}
  ...
  \item
    ...
    \end{enumerate}
  \item Show that, if $ a $ and $ b $ are fixed complex numbers, then $ \abs{z - a} = \abs{z - b} $ describes a line.
...
\begin{enumerate}
  ...
  \hard As we have already seen, complex numbers can be used to solve problems in geometry. For example, multiplication
