...
\begin{figure}
  ...
  \caption[blag]{Babylon in ancient Mesopotamia.\footnotemark}
\end{figure}
...
to find the $ y$-intercepts of a parabola, graphed as \cref{fig:parabola1}
...
\begin{figure}\label{fig:parabola1}
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      xlabel=$x$,
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{The graph of $ y = 11x^2 + 7x - 6.25 $.}
\end{figure}
...
Now suppose we are given a line $ \ell $ passing through the point $ b $ with some slope $ m $; then the angle the line makes with the horizontal
is $ \theta = \tan^{-1} m $, and thus we are simply looking at the line generated by $ \cis \theta $, shifted to $ b $. Suppose we are given a
point $ w $, and we wish to find the perpendicular distance between $ w $ and $ \ell $. This distance is simply
\begin{align*}
  d(w, \ell) &= \abs{(w - b) - \proj_{\cis \theta}(w - b)}\\
             &= \abs{(w - b) - (\cis \theta) \frac{(w - b, \cis \theta)}{(\cis \theta, \cis \theta)}}\\
             &= \abs{(w - b) - (\cis \theta)(w - b, \cis \theta)}\\
             &= \abs{(w - (w, \cis \theta)\cis \theta) - (b - (b, \cis \theta)\cis \theta)}\\
\end{align*}

\subsection*{Exercises}
\begin{enumerate}
  \item Show that, if $ a $ and $ b $ are fixed complex numbers, then $ \abs{z - a} = \abs{z - b} $ describes the line which
        cuts the segment between $ a $ and $ b $ at a right angle.
