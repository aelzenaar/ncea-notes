...
\begin{center}
  \includegraphics[width=0.8\textwidth]{babylon2}\\
  \textit{Tablet BM 13901; image from the British Museum.}
\end{center}
...
gives a proof in \cite{Ste15}, and Artin gives two(!) proofs in \cite{Art91} \S 9.9). All proofs (that I am aware of) require
the use of analysis (i.e. there is no pure-algebra proof).
...
\begin{enumerate}
  ...
  \hard The following inequality, which holds for all real numbers $ a_1 $, $ a_2 $, ..., $ a_n $, $ b_1 $, ..., $ b_n $, is known as the Cauchy-Schwarz
        ...
        \end{enumerate}
  \hard Suppose $ p(x) = \sum_{i = 0}^n a_i x^i $ and $ q(x) = \sum_{i = 0}^m b_i x^i $ are polynomials. Show that $ p(x) q(x) = \sum_{i = 0}^{m + n} c_i x^i $,
