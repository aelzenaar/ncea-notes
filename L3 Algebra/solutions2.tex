...
\filbreak\begin{enumerate}[resume]
  \answer An important identity (known as the \emph{difference of two squares})
...
\filbreak\begin{enumerate}[resume]
  \answer Flesh out the following alternative proof of the quadratic formula. Let
        $ \alpha $ and $ \beta $ be the two roots of the equation $ x^2 + bx + c = 0 $.
        \begin{enumerate}
...
integer $ m $), then it is also the case that $ \alpha + \beta $ and $ \alpha - \beta $ are roots (the roots of $ x^{p^m} - x = 0 $ form
the (unique) finite field of order $ p^m $ denoted $ GF(p^m) $ --- the Galois field of order $ p^m $).}
\end{mdframed}
...
\filbreak\begin{enumerate}[resume]
  \answer Compute $ (6 \cis \frac{23\pi}{24})(9\cis \frac{14\pi}{17}) $ in polar form.
\end{enumerate}
\begin{displaymath}
...
\filbreak\begin{enumerate}[resume]
  \answer Suppose $ x $ is a real number. Show that, for all integers $ n $, $ \cis \frac{2x\pi}{n} = \cis \frac{2(x + n)\pi}{n} $.
\end{enumerate}
We argue as follows:
\begin{align*}
...
\filbreak\begin{enumerate}[resume]
  \answer Show that if $ z = a + ib $ then $ \sqrt{z\bar z} = |z| $ where $ |z| $ is
        the modulus of $ z $.
\end{enumerate}
From question \ref{qn:rationalise}, we have $ z\bar z = a^2 + b^2 $; hence $ \sqrt{z\bar z} = \sqrt{a^2 + b^2} = |z| $.
...
\filbreak\begin{enumerate}[resume]
  \answer If $ z = x + iy $, and $ az^2 + bz + c = 0 $, show that $ a \bar z^2 + b \bar z + c = 0 $
...
\filbreak\begin{enumerate}[resume]
  \answer Use Euler's identity to find $ \ln (-1) $, and hence $ \ln (-x) $ for $ x \in \mathbb{R} $.
\end{enumerate}
\begin{align*}
  e^{i\pi} &= -1\\
  i\pi     &= \ln{(-1)}\\
...
\filbreak\begin{enumerate}[resume]
  \answer Suppose $ | z | = 1 $. Plot all possible values of $ z $ which satisfy this
...
\filbreak\begin{enumerate}[resume]
  \answer Show that, if $ a $ and $ b $ are fixed complex numbers, $ \abs{z - a} = \abs{z - b} $ describes a line.
\end{enumerate}
...
\filbreak\begin{enumerate}[resume]
  \answer Let $ v = 1 + i $ and $ z = x + iy $ for any real numbers $ x $ and $ y $.
    \begin{enumerate}
      \answerb Show that the equation $ | z - v | = | vz | $ represents a circle, and find its
            centre and radius.
      \answerb Find the point of intersection of the circle with the straight line $ | z - v | = | z + v | $.
    \end{enumerate}
\end{enumerate}
...
\begin{mdframed}[hidealllines=true,backgroundcolor=Emerald!20]
\textit{It is beyond the scope of this text to completely prove the following theorem, but we state it here:}
\end{mdframed}

\begin{thm}[Fundamental Theorem of Algebra]
  Let $ p(x) $ be a non-constant polynomial with complex coefficients (i.e. $ \partial p > 0 $). Then there
  exists some complex number $ \zeta $ such that $ p(\zeta) = 0 $.
\end{thm}

The proof of the Fundamental Theorem unfortunately requires concepts and techniques
far beyond the scope of this book (Stewart gives a proof in \textit{Galois Theory}, and Artin
gives two(!) proofs in \textit{Algebra} \S 9.9). All proofs (that I am aware of) require the
use of analysis (i.e. there is no pure-algebra proof).

The first rigorous proof of this theorem was published by the amateur Swiss mathematician Jean-Robert Argand in 1814; the
name is somewhat incorrect in the modern era as the study of algebra is no longer purely devoted to the properties of
the complex number field. An analogue of the theorem in a more modern context is Kronecker's Theorem, which states that
a polynomial with coefficients in any field $ F $ (number system in which we can carry out all four basic arithmetic operations)
has a root in some field $ E $ which contains $ F $.
...
\paragraph{Case I: $ \left(1 - \frac{y_1 - y_0}{x_0 y_1 - x_1 y_0} x_\star - \frac{x_0 - x_1}{x_0 y_1 - x_1 y_0} y_\star \right) = 0 $}
Evidently
\begin{displaymath}
...
We must show that $ f = 0 $. Notice that we have $ ax_0 = -by_0 $ and $ ax_1 = -by_1 $, so we have $ \frac{x_0}{x_1} = \frac{y_0}{y_1} $. Hence
there is some $ m $ such that $ x_0 = m x_1 $ and $ y_0 = m y_1 $. It follows that
\begin{align*}
  dmx_1 + emy_1 + f = 0&\\
  dx_1 + ey_1 + f = 0 &\Rightarrow x_1 = \frac{-f - ey_1}{d}\\
                       &\Rightarrow dm \frac{-f - ey_1}{d} + emy_1 + f = 0\\
                       &\Rightarrow m (-f - ey_1) + emy_1 + f = 0\\
                       &\Rightarrow f(1 - m) = 0
\end{align*}
Now, we know $ m \neq 1 $ since the points are distinct; hence $ f = 0 $ as required,
...
\textit{Many of the results here follow from basic results in number theory. The interested reader is directed
        towards `Elementary Number Theory' by Underwood Dudley. More generally, the material in this chapter
...
roots of unity are of this form by exercise \label{ex:smallprimitive}). Then by B\'ezout's identity we have that there exist $ m $ and $ n $
...
\filbreak\begin{enumerate}[resume]
  \answer Graph the equation $ |z| = 3 $ in the complex plane.
\end{enumerate}
...
\filbreak\begin{enumerate}[resume]
  \answer If $ | z - 1 + 2i | = | z + 1 | $ and $ z = x+yi $, find an expression for $ y $ in terms
...
\filbreak\begin{enumerate}[resume]
  \answer Suppose that $ | z+w | = | z-w | $. Show that $ \carg z - \carg w = \pm \frac{\pi}{2} $. You
        may, if necessary, use the above result that $ \arctan u - \arctan v = \arctan\left(\frac{u-v}{1+uv} \right) $.
\end{enumerate}

\paragraph{Proof using the $ \arctan $ result.}
...
\begin{align*}
  | z + w | = | z - w | &\Rightarrow | a + bi + c + di | = | a + bi - c - di |\\
...
\filbreak\begin{enumerate}[resume]
  \answer If $ 3z^3 + (2-3ai)z^2 + (6+2bi)z + 4 $ is to have exactly one real root, what value must
...
\filbreak\begin{enumerate}[resume]
  \answer Graph the locus of $ \carg w = | w | $. What about $ \carg w + | w | = 1 $?
\end{enumerate}
...
\filbreak\begin{enumerate}[resume]
  \answer An important identity (known as the \emph{sum of two squares})
...
We proceed to show that $ \carg{y_2} = -\carg{y_3} $, and $ |y_2| = |y_1 | $.
...
The fact that $ |y_2| = |y_3| $ is easily shown by drawing a diagram (remembering that $ \carg{\omega} = \frac{2\pi}{3} $ and $ |\omega| = 1 $)
and noticing the symmetry.

\filbreak\begin{enumerate}[resume]
  \answer Find all solutions to $ x^{n - 1} + x^{n - 2} + \cdots + 1 = 0 $, where $ n \in \mathbb{N} $.
\end{enumerate}
...
\filbreak\begin{enumerate}[resume]
  \answer Under which conditions will the equation $ x^2 + a(1+i)x + b(1+i) = 0 $ have one or more
