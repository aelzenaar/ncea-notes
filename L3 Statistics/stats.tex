...
\begin{figure}
  ...
  \begin{tikzpicture}[
    ...
    \begin{axis}[
      ...
      \addplot [fill=purple, fill opacity=0.5] {binom(x,100,0.1)}; \addlegendentry{$p=0.1$};
      \addplot [fill=cyan, fill opacity=0.5] {binom(x,100,0.3)}; \addlegendentry{$p=0.3$};
      \addplot [fill=orange, fill opacity=0.5] {binom(x,100,0.5)}; \addlegendentry{$p=0.5$};
      \addplot [fill=green, fill opacity=0.5] {binom(x,100,0.8)}; \addlegendentry{$p=0.8$};
    \end{axis}
  \end{tikzpicture}
...
We can extend this now to the case where $ g $ is not too wiggly.

\begin{defn}
  A real-valued function $ g $ is called \df{hemiwiggly} on an interval $ (a,b) $ if it is continuously
  differentiable and has only finitely many stationary points on that interval.
\end{defn}

Let $ X $ be a continuous random variable with sample space $ \Omega = (a,b) $ and density $ \rho $. Suppose $ g $ is a hemiwiggly function on $ \Omega $, with
staionary points $ p_1, ..., p_n $. Then for all the

then define the real-valued function $ \pi $ from $ g(\Omega) $ by
\begin{displaymath}
  \pi(y) = \sum_{\substack{x \text{ in } \Omega\\ \text{ such that } g(x) = y}}
\end{displaymath}
