...
Equations and results which appear on the NZQA formula sheet are marked with this symbol: \ding{43}.
...
\begin{proof}[Proof I (Euclid)]
  ...
  \begin{align*}
    \mathcal{A}(BDEA) &= \mathcal{A}(JKBD) + \mathcal{A}(JEAK) && \text{ (Decomposing into rectangles.)}\\
                    ...
                    &= \mathcal{A}(BCFG) + \mathcal{A}(AHIC) && \text{ (Triangle $ BGA $ has height $ BC $ and base $ BG $, similarly for $ BHA $.)}
  \end{align*}
\end{proof}
...
% \begin{exercise}
%   Use the following diagram and an analagous argument to derive the sum formula for sine.\marginsymbol
%   \begin{center}
