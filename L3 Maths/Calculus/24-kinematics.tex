...
\goals{To apply calculus to physics.}
...
\begin{questions}
  ...
  \questioM The velocity $ v $ of an object $ t $ seconds after it moves from the origin is given by
            ...
            \end{displaymath}
    \begin{parts}
      \part Write down the formula for the acceleration of the particle after $ t $ seconds.
      \part Work out the initial velocity and acceleration.
      \part When is the object at rest momentarily?
      \part When did the object return to the origin?
      \part What was the total distance travelled by the object before it returned to the origin?
    \end{parts}
  \questioM A well-wrapped food parcel is dropped from an aeroplane flying at a height of \SI{500}{\metre}
    ...
    \end{parts}
  \questioM A racing car travelling at \SI{210}{\kilo\metre\per\hour} skids for a distance of \SI{150}{\metre}
  ...
  \questioE A ball is thrown straight up from the edge of the roof of a building, with initial velocity $ v_0 $. A second ball is dropped from the
    ...
    \begin{parts}
      \part Suppose the height of the building is \SI{20.00}{\metre}. What must be the initial speed $ v_0 $ if both balls are to hit the ground
            at the same time?
      \part Consider a second building of unknown height $ h $; if the first ball is thrown upwards with initial velocity $ v_1 $, and both
            balls hit the ground at the same time, give an expression for $ h $ in terms of $ v_1 $.
    \end{parts}
\end{questions}
\end{document}
