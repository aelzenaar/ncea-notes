...
\filbreak\begin{enumerate}[resume]
  \answer Show that if $ b $ is a complex number and $ a $ and $ c $ are real numbers, then the locus of all complex numbers
...
\begin{align*}
  0 = a(z,z) + (b,z) + c &= a(z + \frac{b}{2a}, z + \frac{b}{2a}) - \frac{(b,b)}{4a} + c\\
  (z + \frac{b}{2a}, z + \frac{b}{2a}) &= \frac{(b,b) - 4ac}{4a^2}\\
  \sqrt{(z + \frac{b}{2a}, z + \frac{b}{2a})} &= \sqrt{\frac{(b,b) - 4ac}{4a^2}}\\
  \abs{z + \frac{b}{2a}} &= \frac{\sqrt{(b,b) - 4ac}}{2a}.
\end{align*}
So the locus is a circle of radius $ \frac{\sqrt{(b,b) - 4ac}}{2a} $ centred at $ -\frac{b}{2a} $.

\filbreak\begin{enumerate}[resume]
  \answer Let $ X $ be a set of complex numbers. If $ \rho \neq 0 $ and $ t $ are complex numbers, define $ \rho X + t $ to be
        the set of all complex numbers of the form $ \rho x + t $ for some $ x $ in the set $ X $. Show that
    \begin{enumerate}
      \answerb if $ X $ is a line, then $ \rho X + t $ is a line;
      \answerb if $ X $ is a circle, then $ \rho X + t $ is a circle.
    \end{enumerate}
\end{enumerate}

Indeed, if $ X $ is a line then $ X $ is the set of all points of the form $ \alpha z + c $ for some real number $ \alpha $ and
complex number $ c $; so $ \rho X + t $ is the set of all points of the form $ \rho(\alpha z + c) + t = \rho\alpha z + \rho c $,
which is the line in the direction $ \rho z $ through the point $ \rho c $.

If $ X $ is a circle, then $ X $ is the locus of $ \abs{z - c} = r $ for some complex number $ c $ and some point $ r $; then
every point $ w $ in $ \rho X + t $ satisfies $ w = \rho z + t $ for some $ z $ in $ X $, and so $ z = \rho^{-1} (w - t) $;
thus $ \rho X + t $ is the locus of all $ w $ satisfying $ \abs{\rho^{-1} (w - t) - c} = r $. Applying properties of the modulus,
we have that $ \abs{w - (t + \rho c)} = \abs{\rho} r $, and so $ \rho X + t $ is just the circle with centre $ t + \rho c $ with radius $ \abs{\rho} r $.

\filbreak\begin{enumerate}[resume]
  \answer Let $ p $ be a positive real number, and let $ \Gamma $ be the locus of points $ z $ satisfying $ \abs{z - p} = c\realp{z} $. Show
        that $ \Gamma $ is:
    \begin{enumerate}
      \answerb an ellipse if $ 0 < c < 1 $;
      \answerb a parabola if $ c = 1 $;
      \answerb a hyperbola if $ c > 1 $.
    \end{enumerate}
\end{enumerate}

Let $ z = x + yi $. Expanding, we have $ (x - p)^2 + y^2 = c^2x^2 $ and therefore $ (1 - c^2)x^2 - 2px + p^2 + y^2 = 0 $.
If $ c = 1 $, this is clearly the equation of the parabola $ y^2 = 2px - p^2 $; so assume $ c \neq 1 $. Dividing through
by $ 1 - c^2 $ and completing the square, we have
\begin{gather*}
...
\end{gather*}
where the coefficient of the $ (x - p/(1 - c^2))^2 $ term is always positive and the coefficient of the $ y^2 $ term
is positive when $ c < 1 $ and negative when $ c > 1 $, corresponding to an ellipse and a hyperbola respectively.

\filbreak\begin{enumerate}[resume]
  \answer A set of complex numbers is called \emph{convex} if for every pair of complex numbers in the set the line segment joining
        them is also in the set.
    \begin{enumerate}
      \answerb Show that the set of all $ z $ where $ \abs{z} < 1 $ is convex.
      \answerb Show that the set of all $ z $ where $ 0 < \abs{z} < 1 $ is not convex.
    \end{enumerate}
\end{enumerate}
\paragraph{(a)}
Suppose $ w $ and $ v $ lie inside the disc $ \abs{z} < 1 $. Then the segment
joining them is given by $ z = (1 - t)w + tv $ (where $ 0 \leq t \leq 1 $).
Computing the modulus of this, we obtain
\begin{displaymath}
  \abs{z} = \abs{(1 - t)w + tv} \leq \abs{(1 - t)w} + \abs{tv} = (1-t)\abs{w} + t\abs{v} < (1 - t) + t = 1;
\end{displaymath}
so every point in the segment also lies inside the disc.

\paragraph{(b)}
Consider the points $ w = -1/2 $ and $ v = 1/2 $. Then the segment between them is given by $ z = (1 - t)w + tv $, so
the point $ (1 - (1/2))w + (1/2)v = (1/2)(-1/2) + (1/2)(1/2) = 0 $ lies on the segment but not inside the given set.

\filbreak\begin{enumerate}[resume]
  \answer A set of complex numbers is called \emph{star shaped} if there is some point $ c $ in the set such that for every
        other point $ p $ in the set, the segment joining $ p $ to $ c $ lies in the set. Show that every convex set is star shaped.
\end{enumerate}
If a set is convex, then pick any point $ c $ in the set; then the segment joining $ p $ to $ c $ lies inside the set (where $ p $ is
any other point in the set) and so the set is star-shaped.

\filbreak\begin{enumerate}[resume]
  \answer If $ S $ is a set of finitely many complex numbers $ z_1, ..., z_n $, the \emph{convex hull} of $ S $ is the smallest convex set
...
\end{enumerate}
This is proved in two steps: we show that the given set is convex, and that it is contained in every other convex set containing $ S $ (so
it is the smallest such set).
...
\begin{mdframed}[hidealllines=true,backgroundcolor=Emerald!20]
\textit{More generally, the convex hull of any set of points is the smallest convex set containing those points, and
        we have that the set of all points in the convex hull is the set of finite convex combinations of points in
        the set. However, it turns out that if we take our points in $ n$-dimensional space then the convex hull of
        a set can be described by the convex combinations of sets of $ n + 1 $ points in the set. This theorem is
        due to Carath\'eodory (see \cite{Ber94}, chapter 11).}
\end{mdframed}
...
\filbreak\begin{enumerate}[resume]
  \answer The following inequality, which holds for all real numbers $ a_1 $, $ a_2 $, ..., $ a_n $, $ b_1 $, ..., $ b_n $, is known as the Cauchy-Schwarz
        ...
        \begin{enumerate}
          ...
          \answerb As a special case of the inequality, show that if $ w $ and $ z $ are complex numbers then $ (w, z)^2 \leq \abs{w}^2 \abs{z}^2 $.
          \answerb Hence show the \emph{triangle inequality}: $ \abs{a + b} \leq \abs{a} + \abs{b} $ for complex numbers $ a $ and $ b $.
        \end{enumerate}
\end{enumerate}
...
\filbreak\begin{enumerate}[resume]
  \answer Multiplication by $ i $ rotates a point by $ \frac{\pi}{2} $ around the origin.
        \begin{enumerate}
          \answerb A generalisation of this allows us to rotate a point $ z $ around an arbitrary point $ a $ by that
                angle: $ z' = a + i(z-a) $. Justify this formula.
          \answerb Consider a treasure map with the following instructions:
...
\paragraph{(a)}
We shift our coordinate system so that $ a $ becomes the origin, rotate the plane by multiplying by $ i $, and shift the origin back to $ a $.
...
\filbreak\begin{enumerate}[resume]
  \answer A \emph{Gaussian integer} is a complex number of the form $ m + ni $, for integers $ m $ and $ n $. Show that there is
        no equilaterial triangle on the complex plane with Gaussian integer vertices.
\end{enumerate}
Suppose we have such an equilateral triangle; without loss of generality, we can take one of the vertices to be the origin. Let
the other two vertices be $ a + bi $ and $ c + di $. Since the triangle is equilateral, $ \abs{a + bi} = \abs{c + di} $. Further,
the angle between the two points at the origin is $ \frac{\pi}{3} $. Then, by the rotation property of complex multiplication,
\begin{displaymath}
  a + bi = \cis \frac{\pi}{3} (c + di) = \frac{1}{2}c + \frac{\sqrt{3}}{2} di;
\end{displaymath}
...
\begin{center}
  \includegraphics[width=0.5\textwidth]{roses}
\end{center}
