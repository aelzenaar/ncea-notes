...
The $ x$-intercepts of the curve are $ x = \pm a $, so the chord along the $ x$-axis has length $ 2a $.

Suppose the sign is $ + $, so we have an ellipse. Then the $ y$-intercepts are $ y = \pm a $, so the $ y$-axis chord has length $ 2b $. If we have
a hyperbola, then we don't have any $ y$-intercepts (since such an intercept would satisfy $ (y/b)^2 = -1 $, which is impossible).
