...
\begin{questions}
  \question Graph the equation $ y = \vartheta^x $ for different values of $ \vartheta $:
    ...
    \begin{parts}
      ...
      \part When $ \vartheta $ is negative, the curve is an \emph{exponential decay} curve; when $ \vartheta $ is positive,
the curve
  ...
  \question A graph with Cartesian equation of the form $ y = a(x - x_0)^{-1} + c $ is a \emph{hyperbola}.
    ...
    \end{parts}
  \question Challenge question. Consider the equation $ 6^{2x} + m\cdot 6^x + n = 0 $, where $ n \leq 0 $.
    \begin{parts}
      \part Prove that the equation has precisely two solutions for $ 6^x $.
      \part Show that only one of these solutions is valid for finding a solution for $ x $ if $ m $ is positive.
    \end{parts}
\end{questions}
