...
\begin{questions}
  ...
  \question This question is a list of mechanical exercises. It is important to be fluent
    ...
    \begin{parts}
      ...
      \part Write $ \log_2 \sqrt[3]{\frac{\sqrt[3]{15} \thinspace 5^4}{3^3 \thinspace \sqrt[3]{9}}} $ as a single number.
      ...
      \part Solve for $ x $ if $ 210 = \left(10^x\right)^3 $.
  ...
  \question Give the equations of the following exponential graphs:
    ...
    \fbox{\begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[domain = -2:4, color = blue, samples = 100] {2^x};
      \end{axis}
    ...
    \fbox{\begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[domain = -2:4, color = purple, samples = 100] {0.5*3^x};
      \end{axis}
    \end{tikzpicture}}\\
    \fbox{\begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[domain = -2:4, color = orange, samples = 100] {-2^(-x-1)};
      \end{axis}
    \end{tikzpicture}}\quad
    \fbox{\begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[domain = -2:8, color = green, samples = 100] {-1/3 * 4^(x-3) - 7};
      \end{axis}
    \end{tikzpicture}}
    \end{center}
  ...
  \question Many population models are exponential.
    ...
    \end{parts}
  \question Radioactive substances slowly decompose into lead. The rate at which the decomposition occurs is usually measured in terms
            of the \emph{half-life}: the time taken for an amount of the substance to decay by a factor of $ \frac12 $. It can be shown
            (using calculus) that if $ A_0 $ is the amount of material initially present, then the material present after $ t $ years can be modelled by
            \begin{displaymath}
              A(t) = A_0 (2.72)^{-kt}
            \end{displaymath}
            where $ k $ is a constant.
    \begin{parts}
      \part Show that $ k = \frac{\log_{2.72} 2}{\tau} $, where $ \tau $ is the half-life in years.
      \part Radium has a half-life of \SI{1590}{\year}. How long will it take for six grams of radium to decay to one gram?
      \part A new radioactive substance decays from 1 gram to 0.98 grammes in one year. What is the half-life of the substance?
      \part One of the most useful applications of this model is in dating ancient materials. Because of the action of cosmic
            rays in the atmosphere, there is always a certain percentage of a particular radioactive form of carbon in the atmosphere
            (namely, carbon-14). Since living matter is always interchanging material with the environment, the living matter also
            contains the same percentage of carbon-14. Upon death, the exchange with the environment ceases and so no new carbon-14
            enters cells; the remaining carbon-14 continues to decay, and the amount of remaining carbon-14 can be detected using
            instrumentation. The model can then be used to estimate the rough date of death.
