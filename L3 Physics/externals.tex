...
In other words, the answer to the title question is `your entire life until this point'.
...
Now, suppose that a force acts on an object, causing it to accelerate from a speed $ v_0 $ to a speed $ v_1 $ (one example could
be a falling rock accelerating from a speed of \SI{0}{\metre\per\second} to some final velocity at impact). A natural question to
ask is `how much force did we put into our system in order to move that object', or `how much work did it take'? We wish to sum
together all of the instantaneous forces which we apply over the distance from $ x_0 $ to $ x_1 $; this is just an integral:
\begin{displaymath}
...
\begin{exercise}
  ...
  mass of the moon is \SI{7.3e22}{\kilo\gram}. What is the velocity of the orbit of the satellite?
\end{exercise}
...
\begin{equation}
  V = \frac{\Delta U}{q}.
\end{equation}
Since $ \Delta U = W = -F \Delta x = -Eq \Delta x $\footnote{~There is a minus sign here because the particle gains energy as it
works against a force; that is, when a positive charge moves in the direction of a force --- $ \Delta x $ and $ F $ have the same
sign --- the particle loses potential energy and so $ \Delta U $ must be negative. [This footnote was added after the author spent
an embarassingly long time wondering where the magic minus sign came from.]} in a uniform field $ E $ (where $ \Delta x $ is the
displacement of the charge), we have
\begin{gather}
...
if you are more interested in the applied sciences, and they tend to offer more scholarships.
